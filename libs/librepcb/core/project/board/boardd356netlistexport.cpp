...
QByteArray BoardD356NetlistExport::generate() const {
  D356NetlistGenerator gen(*mBoard.getProject().getMetadata().getName(),
                           mBoard.getProject().getMetadata().getVersion(),
                           *mBoard.getName(), mCreationDateTime);

  // Vias.
  foreach (const BI_NetSegment* segment, mBoard.getNetSegments()) {
    const QString netName = *segment->getNetSignal().getName();
